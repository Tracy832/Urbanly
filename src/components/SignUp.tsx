...
const SignUp: React.FC = () => {
  ...
  const handleSignUp = (e: React.FormEvent) => {
    e.preventDefault();
    // 1. SAVE DATA: Store the name so the Dashboard can say "Welcome, Tracy"
    localStorage.setItem('user_name', fullName);
    
    // 2. PATH: Auth -> Welcome (The starting point of the journey)
    navigate('/'); 
  };
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          <form className="space-y-6" onSubmit={handleSignUp}>
            ...
            <div className="space-y-2">
              ...
              <input required type="email" placeholder="tracy@urbanly.co.ke" className="w-full p-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-urban-sand/20 outline-none transition-all font-bold" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-urban-sand">Password</label>
              <input required type="password" placeholder="••••••••" className="w-full p-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-urban-sand/20 outline-none transition-all font-bold" />
            </div>
